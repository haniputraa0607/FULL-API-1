...
class TransactionPickupGoSend extends Model
{
	...
	public function book($fromRetry = false, &$errors = []) {
		$trx = Transaction::join('transaction_pickups', 'transaction_pickups.id_transaction', 'transactions.id_transaction')->where('id_transaction_pickup', $this->id_transaction_pickup)->with('outlet')->get();
