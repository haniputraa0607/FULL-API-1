    ...
    function outletAvailable($user, $id_deals_user, $outlet_code)
    {
        ...
        if(!empty($dataDeals)){
            if($dataDeals['is_all_outlet'] == 1){
                $checkOutlet = Outlet::where('outlet_code', strtoupper($outlet_code))->where('outlet_status', 'Active')
                    ->first();
                if($checkOutlet){
                    $deals = DealsUser::join('deals_vouchers', 'deals_vouchers.id_deals_voucher', '=', 'deals_users.id_deals_voucher')
                        ...
                        ->addSelect(DB::raw('*,deals_users.id_outlet as deals_user_outlet, deals_users.id_outlet as check_outlet_deals_user,((deals_users.id_outlet is null) or deals_users.id_outlet = '.$checkOutlet['id_outlet'].') as status_outlet'))
                        ->first();

                    if($deals){
                        $deals['outlet_name'] = $checkOutlet['outlet_name'];
                        $deals['outlet_code'] = $checkOutlet['outlet_code'];
                    }
                }else{
                    $deals = [];
                }
