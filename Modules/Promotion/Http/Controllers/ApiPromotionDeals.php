...
class ApiPromotionDeals extends Controller
{
    ...
    public function detail(DetailPromotion $request)
    {
		...
		$deals = DealsPromotionTemplate::orderBy('deals_promotion_templates.updated_at', 'desc')
				...
				->with([
	                ...
	                'deals_promotion_buyxgety_product_requirement',
    ...
    public function checkComplete($dataDeals, &$step, &$errors)
    {
    	...
    	{
	    	if ( empty($deals['deals_promotion_product_discount_rules']) && empty($deals['deals_promotion_tier_discount_rules']) && empty($deals['deals_promotion_buyxgety_rules']) && empty($deals['deals_promotion_discount_delivery_rules']) )
	    	{
	    		$step = 2;
