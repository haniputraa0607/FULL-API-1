		...
		if($query){
			...
			if($crm['autocrm_push_toogle'] == 1 && !$forward_only){
				if(!empty($user['phone'])){
					try {
                        ...
                        }elseif ($crm['autocrm_push_clickto'] == 'Referral') {
                            $dataOptional['id_reference'] = 0;
                        ...
                        //======set id reference when in autocrm not null
			...
			if($crm['autocrm_inbox_toogle'] == 1 && !$forward_only){
				if(!empty($user['id'])){
                    ...
                    }elseif ($crm['autocrm_inbox_clickto'] == 'Outlet') {
                        if (isset($variables['id_outlet'])) {
                            $inbox['inboxes_id_reference'] = $variables['id_outlet'];
                        }else{
                            $inbox['inboxes_id_reference'] = 0;
                        }
                    }elseif ($crm['autocrm_inbox_clickto'] == 'Order') {
                        if (isset($variables['id_outlet'])) {
                            $inbox['inboxes_id_reference'] = $variables['id_outlet'];
                        }else{
                            $inbox['inboxes_id_reference'] = 0;
                        }
                    }elseif ($crm['autocrm_inbox_clickto'] == 'Referral') {
                        $inbox['inboxes_id_reference'] = 0;
			...
			foreach($query as $replace){
				...
				if($replace['type'] == 'String'){
					if($replace['reference'] == 'variables'){
						...
						if($replace['keyword'] == '%first_name%'){
                            $firstName = explode(" ",$user['name']);
                            $replaced = $firstName[0]??$user['name'];
                        }else if($user[$replace['reference']] != ""){
