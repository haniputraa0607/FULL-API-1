...
class ApiNews extends Controller
{
    ...
    function listNews(Request $request) {
            ...
            if(!isset($post['id_news'])){
                $news->select('id_news','id_news_category','news_title', 'news_post_date', 'news_publish_date','news_expired_date','news_slug','news_content_short','news_image_luar','news_image_dalam', DB::raw("DATE_FORMAT(news_post_date, '%W, %d %M %Y %H:%i') as news_post_date"));
            }else{
