		...
		foreach($current as $cur){
			...
			foreach($post['membership'] as $i => $membership){
				...
				if($cur['id_membership'] == $membership['id_membership']){
					...
					$data['next_level_text'] = $membership['next_level_text']??'Get more transaction!<br>%value% Points to go to become <b>%membership% Member</b>';
	...
	public function calculateMembership($phone) {
		...
		if($check){
			...
			if(!empty($check['retain_date'])){
				...
				$total_balance = LogBalance::whereIn('source', ['Transaction'])
											->where('balance', '>', 0)
		...
		foreach($users as $datauser){
			...
			foreach($membership_all as $all){
				...
				switch ($all['membership_type']) {
					...
					case 'balance':
						$total_balance = LogBalance::whereIn('source', ['Transaction'])
						->where('id_user',$datauser->id)->where('balance', '>', 0)->sum('balance');
						if($total_balance >= $all['min_total_balance']){
							//level up
							$membership_baru = $all;
						}
