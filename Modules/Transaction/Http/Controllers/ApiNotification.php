...
class ApiNotification extends Controller {
    ...
    public function logReceiveNotification(Request $request) {
        $midtrans = $request->json()->all();
        $response = $this->receiveNotification($request);
        try {
            LogMidtrans::create([
                'type'                 => 'notification',
    ...
    public function notificationPending($mid, $trx)
    {
        ...
        $send = app($this->autocrm)->SendAutoCRM('Transaction Payment', $trx->user->phone, [
            'notif_type' => 'trx',
            'header_label' => $title,
            'date' => date('d M Y H:i', strtotime($trx['transaction_date'])),
...
Detail: ".$link['short'],
    ...
    public function adminOutletConfirm(Request $request) {
        $post = $request->json()->all();
        // return $post;
